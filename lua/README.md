...
The Lua SDK for the ProfanityChecker API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:CheckProfanity()` — each with the same small set of operations (`create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/profanity-checker-sdk/releases)),
...
```lua
local checkprofanity, err = client:CheckProfanity():create({ message = "example" })
if err then error(err) end
```
...
```
lua/
├── profanity-checker_sdk.lua    -- Main SDK module
