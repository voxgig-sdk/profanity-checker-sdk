...
require_relative 'ProfanityChecker_types'
...
class ProfanityCheckerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ProfanityCheckerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ProfanityCheckerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ProfanityCheckerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ProfanityCheckerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ProfanityCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ProfanityCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ProfanityCheckerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ProfanityCheckerError => err
    ...
    if fetched.is_a?(Hash)
      status = ProfanityCheckerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ProfanityCheckerError.new(
        "graphql_error", "ProfanityCheckerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ProfanityCheckerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
