# ProfanityChecker Ruby SDK

The Ruby SDK for the ProfanityChecker API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.CheckProfanity(nil).create({ "name" => "Example" }, nil)
...
result, err = client.ProfanityChecker(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── ProfanityChecker_sdk.rb       -- Main SDK module
