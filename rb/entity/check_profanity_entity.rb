...
class CheckProfanityEntity
  ...
  def make
    opts = @_entopts.dup
    CheckProfanityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ProfanityCheckerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ProfanityCheckerHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ProfanityCheckerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
