...
func TestCheckProfanityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		checkProfanityRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "check_profanity"}), "check_profanity_ref01"))

		checkProfanityRef01DataResult, err := checkProfanityRef01Ent.Create(checkProfanityRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		checkProfanityRef01Data = core.ToMapAny(entityData(checkProfanityRef01DataResult))
...
func check_profanityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PROFANITY_CHECKER_TEST_CHECK_PROFANITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PROFANITY_CHECKER_TEST_CHECK_PROFANITY_ENTID"])
