...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PROFANITY_CHECKER_TEST_LIVE") == "TRUE" ||
		os.Getenv("PROFANITY_CHECKER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PROFANITY_CHECKER_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.ProfanityCheckerSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ProfanityCheckerSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ProfanityCheckerError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ProfanityCheckerSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ProfanityCheckerError{Msg: msg, Code: code}
}
