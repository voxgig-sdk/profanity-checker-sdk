...
import (
	...
	omni "github.com/voxgig-sdk/profanity-checker-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ProfanityCheckerError); is {
				out := map[string]any{
					"name":    "ProfanityCheckerError",
...
func omniCtx(arg any, client *sdk.ProfanityCheckerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ProfanityCheckerSDK); is {
			client = live
