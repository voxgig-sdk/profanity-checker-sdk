...
from projectname_sdk.core.control import ProfanityCheckerControl
...
class ProfanityCheckerContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ProfanityCheckerControl()
        ...
        if isinstance(sp, ProfanityCheckerSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ProfanityCheckerResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ProfanityCheckerResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ProfanityCheckerOperation({})
    ...
    def make_error(self, code, msg):
        return ProfanityCheckerError(code, msg, self)
