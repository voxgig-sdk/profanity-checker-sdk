...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ProfanityCheckerOmni.make_runner(TEST_JSON_FILE, ProfanityCheckerSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    ProfanityCheckerError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ProfanityCheckerOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = ProfanityCheckerOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
