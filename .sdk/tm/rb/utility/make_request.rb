...
module ProfanityCheckerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ProfanityCheckerResponse.new({})
    ...
    if fetch_err
      ...
      response = ProfanityCheckerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ProfanityCheckerResponse.new(fetched)
    else
