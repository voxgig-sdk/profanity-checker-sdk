...
// ProfanityChecker SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ProfanityCheckerTestFeature extends ProfanityCheckerBaseFeature
{
    ...
    public function init(ProfanityCheckerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ProfanityCheckerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ProfanityCheckerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ProfanityCheckerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ProfanityCheckerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ProfanityCheckerParam::call($ctx, $k);
