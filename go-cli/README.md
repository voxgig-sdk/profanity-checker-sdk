# profanity-checker-cli

boru-driven command-line client **and** interactive REPL for the ProfanityChecker
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/profanity-checker-cli)
make build

# 2. See usage (words, entities, env vars)
./profanity-checker-cli --help
...
PROFANITY_CHECKER_BASE=https://api.example.com ./profanity-checker-cli --help

# 6. No arguments -> interactive REPL
./profanity-checker-cli
profanity-checker> /help
profanity-checker> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/profanity-checker-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/profanity-checker-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `profanity-checker>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `check_profanity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/profanity-checker-cli`. |
...
(`./profanity-checker-cli <expr>`) and interactively (the REPL), and expressions compose the
