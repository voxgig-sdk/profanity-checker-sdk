...
//     (\`new ProfanityCheckerSDK(...)\` / \`ProfanityCheckerSDK.test(...)\` become
//     \`ProfanityCheckerSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ProfanityCheckerSDK'
...
const TEST_SEED = {"entity":{"check_profanity":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ProfanityCheckerSDK(...)\` and any
// \`ProfanityCheckerSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ProfanityCheckerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ProfanityCheckerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
