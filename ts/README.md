...
The TypeScript SDK for the ProfanityChecker API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.CheckProfanity()` — each with a small set of operations (`create`)
...
```ts
try {
  const checkprofanity = await client.CheckProfanity().create({ message: "example" })
...
await checkprofanity.create({ message: "example" })
