...
MCP server exposing the ProfanityChecker SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `profanity-checker_list` | `entity`, optional `query` map | First page of records as JSON |
| `profanity-checker_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o profanity-checker-mcp ./...
```
...
After install, restart Claude Code; the `profanity-checker_list` and
`profanity-checker_load` tools become available in new sessions.
