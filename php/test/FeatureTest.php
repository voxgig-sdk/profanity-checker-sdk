...
// ProfanityChecker SDK feature test
//
...
class FtCtrl extends ProfanityCheckerControl
{
...
class FtClient
{
    ...
    private ProfanityCheckerUtility $utility;

    public function __construct(ProfanityCheckerUtility $utility, array $options)
    {
    ...
    public function get_utility(): ProfanityCheckerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ProfanityCheckerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ProfanityCheckerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ProfanityCheckerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ProfanityCheckerConfig::make_feature($name);
    ...
    private static function buildUrl(ProfanityCheckerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ProfanityCheckerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ProfanityCheckerResponse($fetched);
        }
    ...
    private function fail(ProfanityCheckerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ProfanityCheckerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
