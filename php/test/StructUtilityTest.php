...
// ProfanityChecker SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ProfanityCheckerOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ProfanityCheckerOmni::structfix($now, Struct::undef()))
                        !== json_encode(ProfanityCheckerOmni::structfix($val, Struct::undef()))) {
