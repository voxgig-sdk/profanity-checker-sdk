...
// ProfanityChecker SDK pipeline test
//
...
{
    private static function utility(): ProfanityCheckerUtility
    {
        return new ProfanityCheckerUtility();
    }
    ...
    private static function ctx(array $over = []): ProfanityCheckerContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ProfanityCheckerContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ProfanityCheckerOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ProfanityCheckerError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ProfanityCheckerOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ProfanityCheckerMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ProfanityCheckerOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ProfanityCheckerMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ProfanityCheckerOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ProfanityCheckerMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProfanityCheckerSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ProfanityCheckerMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ProfanityCheckerResponse([]),
            'result' => new ProfanityCheckerResult([]),
        ]);
        [, $err] = ProfanityCheckerMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec([]),
            'response' => null,
            'result' => new ProfanityCheckerResult([]),
        ]);
        [, $err] = ProfanityCheckerMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec(['step' => 's']),
            'response' => new ProfanityCheckerResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ProfanityCheckerResult([]),
        ]);
        [, $err] = ProfanityCheckerMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec(['step' => 's']),
            'response' => new ProfanityCheckerResponse(self::resp(200, ['v' => 1])),
            'result' => new ProfanityCheckerResult([]),
        ]);
        [, $err] = ProfanityCheckerMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec(['step' => 's']),
            'response' => new ProfanityCheckerResponse(self::resp(200, ['v' => 2])),
            'result' => new ProfanityCheckerResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ProfanityCheckerMakeResponse::call($ctx);
    ...
    {
        $preset = new ProfanityCheckerResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec([]),
            'response' => new ProfanityCheckerResponse([]),
            'result' => new ProfanityCheckerResult([]),
        ...
        [$got, $err] = ProfanityCheckerMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ProfanityCheckerResult([])]);
        [, $err] = ProfanityCheckerMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ProfanityCheckerSpec([]), 'result' => null]);
        [, $err] = ProfanityCheckerMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProfanityCheckerSpec(['step' => 's']),
            'result' => new ProfanityCheckerResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ProfanityCheckerOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProfanityCheckerMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProfanityCheckerSpec(['step' => 's']),
            'result' => new ProfanityCheckerResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ProfanityCheckerOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProfanityCheckerMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProfanityCheckerResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ProfanityCheckerSpec([]),
            'result' => new ProfanityCheckerResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ProfanityCheckerMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProfanityCheckerMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ProfanityCheckerError('boom', 'boom');
        $utility->fetcher = function (ProfanityCheckerContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProfanityCheckerSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ProfanityCheckerMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ProfanityCheckerContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ProfanityCheckerMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ProfanityCheckerResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ProfanityCheckerMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ProfanityCheckerContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProfanityCheckerSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ProfanityCheckerMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ProfanityCheckerResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ProfanityCheckerSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ProfanityCheckerMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ProfanityCheckerMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ProfanityCheckerMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ProfanityCheckerResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ProfanityCheckerDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ProfanityCheckerResult(['ok' => false])]);
        $this->expectException(ProfanityCheckerError::class);
        ProfanityCheckerDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ProfanityCheckerResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ProfanityCheckerMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ProfanityCheckerMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ProfanityCheckerBaseFeature();
        $b = new ProfanityCheckerBaseFeature();
        ProfanityCheckerFeatureAdd::call($ctx, $a);
        ProfanityCheckerFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ProfanityCheckerBaseFeature
    {
        $f = new ProfanityCheckerBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ProfanityCheckerFeatureAdd::call($ctx, self::named_feature('a'));
        ProfanityCheckerFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ProfanityCheckerFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ProfanityCheckerFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ProfanityCheckerFeatureAdd::call($ctx, $replace);
        ...
        ProfanityCheckerFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ProfanityCheckerContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ProfanityCheckerSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProfanityCheckerPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ProfanityCheckerPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ProfanityCheckerResponse(['headers' => null]),
            'result' => new ProfanityCheckerResult([]),
        ]);
        ProfanityCheckerResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ProfanityCheckerResultBody::call($ctx);
