...
// ProfanityChecker SDK retry feature
...
class ProfanityCheckerRetryFeature extends ProfanityCheckerBaseFeature
{
    ...
    public function init(ProfanityCheckerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ProfanityCheckerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ProfanityCheckerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ProfanityCheckerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
