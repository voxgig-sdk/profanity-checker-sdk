...
class ProfanityCheckerSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new ProfanityCheckerUtility();
        ...
        $feature_opts = ProfanityCheckerHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ProfanityCheckerHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ProfanityCheckerFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ProfanityCheckerFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ProfanityCheckerUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ProfanityCheckerHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ProfanityCheckerHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ProfanityCheckerHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ProfanityCheckerError($op . "_allow",
                "ProfanityCheckerSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ProfanityCheckerHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ProfanityCheckerError("graphql_error",
                "ProfanityCheckerSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function CheckProfanity($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_check_profanity === null) {
                $this->_check_profanity = new CheckProfanityEntity($this, null);
            }
            return $this->_check_profanity;
        }
        return new CheckProfanityEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ProfanityCheckerSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
